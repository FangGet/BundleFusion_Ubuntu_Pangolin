...
void SIFTImageManager::AddCurrToResidualsCU(unsigned int curFrame, unsigned int startFrame, unsigned int numFrames, const float4x4& colorIntrinsicsInv) {
	...
	int temp = 0;
	cutilSafeCall(cudaMemcpy(&temp, d_globNumResiduals, sizeof(unsigned int), cudaMemcpyDeviceToHost));
...
void __global__ InvalidResidualsCU_Kernel(EntryJ* d_globMatches, unsigned int globNumResiduals, unsigned int removeImgIdx)
{
	const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
		
	if (idx < globNumResiduals) 
	{
		if (d_globMatches[idx].imgIdx_i == removeImgIdx ||
			d_globMatches[idx].imgIdx_j == removeImgIdx) 
		{
...
void SIFTImageManager::RemoveInResidualsCU(unsigned int removeImgIdx, unsigned int removeNumIndex) {
	unsigned int removeNum = m_currNumMatches[removeNumIndex];
	//if (removeNum == 0) return;

	//remove old match
	m_globNumResiduals = m_globNumResiduals - removeNum;
	MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_globMatches, d_globMatches + removeNum, sizeof(EntryJ)*m_globNumResiduals, cudaMemcpyDeviceToDevice));
	MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_globNumResiduals, &m_globNumResiduals, sizeof(unsigned int), cudaMemcpyHostToDevice));
	
	const unsigned int N = m_globNumResiduals;
	const unsigned int threadsPerBlock = INVALIDATEIMAGE_TO_IMAGE_KERNEL_THREADS_X;
	//Invalid other match related to old match
	InvalidResidualsCU_Kernel << <(N + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock >> >(d_globMatches, m_globNumResiduals, removeImgIdx);

	
}

void __global__ processResidualsCU_Kernel(EntryJ* d_globMatches, unsigned int globNumResiduals, unsigned int leftBias)
{
	const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
		
	if (idx < globNumResiduals) 
	{
		if(d_globMatches[idx].isValid()){
			d_globMatches[idx].imgIdx_i -=leftBias;
			d_globMatches[idx].imgIdx_j -=leftBias;
		}
	}
...
void SIFTImageManager::processResidualsCU(unsigned int leftBias) {
	if (leftBias == 0) return;

	const unsigned int N = m_globNumResiduals;
	const unsigned int threadsPerBlock = INVALIDATEIMAGE_TO_IMAGE_KERNEL_THREADS_X;
	//fix each match imgIdx = imgIdx - leftBias
	processResidualsCU_Kernel << <(N + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock >> >(d_globMatches, m_globNumResiduals, leftBias);
