...
public:
	...
	bool hasFaces() const{ return m_faces.size() > 0;}
...
// 		if (m_faces.size() > 0 && m_faces.size() != m_points.size())	is = false;
		return is;
	}
	...
	void merge(const PointCloud<FloatType>& other) {
		...
		if(hasFaces() != other.hasFaces()){
			MLIB_WARNING("faces deleted");
			m_faces.clear();
		}
		...
		size_t facesBefore = m_faces.size();
		...
		if (hasFaces() || isEmpty())	m_faces.insert(m_faces.end(), other.m_faces.begin(), other.m_faces.end());
	}
	...
	void sparsify(size_t newMaxCount) {
		...
		while (m_points.size() > newMaxCount) {
			...
			if (hasFaces()) {
				std::swap(m_faces[n], m_faces.back());
				m_faces.pop_back();
			}
		}
	}
