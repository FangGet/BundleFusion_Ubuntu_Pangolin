...
extern "C" void buildVariablesToCorrespondencesTableCUDA(EntryJ* d_correspondences, unsigned int numberOfCorrespondences, unsigned int maxNumCorrespondencesPerImage, int* d_variablesToCorrespondences, int* d_numEntriesPerRow, CUDATimer* timer, unsigned int bias = 0);
...
void CUDASolverBundling::solve(EntryJ* d_correspondences, unsigned int numberOfCorrespondences, const int* d_validImages, unsigned int numberOfImages,
	...
	bool rebuildJT, bool findMaxResidual, unsigned int revalidateIdx, bool isLocal)
{
	...
	if (rebuildJT) {
#ifdef TEST_TABLE
		if (!isLocal && numberOfImages> 10){
			buildVariablesToCorrespondencesTable(d_correspondences, numberOfCorrespondences, numberOfImages - 10 );
		}
		else{
		...
		/*if (!isLocal){
			...
			outfile.open("table.txt", std::ios::app);
			outfile << "==============" << numberOfCorrespondences << "================" << std::endl<<"0: ";
			std::vector<int> temp;
			temp.resize(20 * m_maxCorrPerImage);
			cutilSafeCall(cudaMemcpy(temp.data(), d_variablesToCorrespondences, sizeof(int) * temp.size(), cudaMemcpyDeviceToHost));
			int lineNum = 0; int before = -2;
			for each(int a in temp){
				if (before == -1 && a == -1) continue;
				if (a == -1) {
					before = a;
					lineNum++; outfile << std::endl << lineNum<<": ";
					continue;
				}
				before = a;
				outfile << a << " ";
...
void CUDASolverBundling::buildVariablesToCorrespondencesTable(EntryJ* d_correspondences, unsigned int numberOfCorrespondences, unsigned int bias)
{
	...
	if (numberOfCorrespondences > 0)
		buildVariablesToCorrespondencesTableCUDA(d_correspondences, numberOfCorrespondences, m_maxCorrPerImage, d_variablesToCorrespondences, d_numEntriesPerRow, m_timer, bias);
