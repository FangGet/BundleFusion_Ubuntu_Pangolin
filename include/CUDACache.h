...
public:
	...
	CUDACache(unsigned int widthDepthInput, unsigned int heightDepthInput, unsigned int widthDownSampled, unsigned int heightDownSampled, unsigned int maxNumImages, const mat4f& inputIntrinsics, bool isLocal = true);
	~CUDACache() {
		free();
	}
	...
	void copyCacheFrameFrom(CUDACache* other, unsigned int frameFrom, bool isLocal = true) {
