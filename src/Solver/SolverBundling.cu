...
__global__ void BuildVariablesToCorrespondencesTableDevice(EntryJ* d_correspondences, unsigned int numberOfCorrespondences,
	unsigned int maxNumCorrespondencesPerImage, int* d_variablesToCorrespondences, int* d_numEntriesPerRow, unsigned int bias =0)
{
	...
	if (x < N) {
		...
		if (corr.isValid()) {
			if(corr.imgIdx_i - bias >=0 && corr.imgIdx_j - bias >=0){
				int offset0 = atomicAdd(&d_numEntriesPerRow[corr.imgIdx_i - bias], 1); // may overflow - need to check when read
				int offset1 = atomicAdd(&d_numEntriesPerRow[corr.imgIdx_j - bias], 1); // may overflow - need to check when read
				if (offset0 < maxNumCorrespondencesPerImage && offset1 < maxNumCorrespondencesPerImage)	{
					d_variablesToCorrespondences[(corr.imgIdx_i-bias) * maxNumCorrespondencesPerImage + offset0] = x;
					d_variablesToCorrespondences[(corr.imgIdx_j-bias) * maxNumCorrespondencesPerImage + offset1] = x;
				}
				else { //invalidate
				...
				printf("INVALIDATING %d(%d,%d)",x, corr.imgIdx_i, corr.imgIdx_j);
			}
		}
	}
}

extern "C" void buildVariablesToCorrespondencesTableCUDA(EntryJ* d_correspondences, unsigned int numberOfCorrespondences, unsigned int maxNumCorrespondencesPerImage, int* d_variablesToCorrespondences, int* d_numEntriesPerRow, CUDATimer* timer, unsigned int bias)
{
	const unsigned int N = numberOfCorrespondences;

	if (timer) timer->startEvent(__FUNCTION__);

	BuildVariablesToCorrespondencesTableDevice << <(N + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK >> >(d_correspondences, numberOfCorrespondences, maxNumCorrespondencesPerImage, d_variablesToCorrespondences, d_numEntriesPerRow, bias);
